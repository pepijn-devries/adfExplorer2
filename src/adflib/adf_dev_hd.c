...
RETCODE adfMountHdFile ( struct AdfDevice * const dev )
{
    ...
    if ( dev->volList == NULL ) {
        //(*adfEnv.eFct)("adfMountHdFile : malloc"); /*Edit PdV*/
        return RC_MALLOC;
    }
...
/*printf("root=%ld\n",vol->rootBlock);*/
    ...
    if (vol->rootBlock==1) {
        //(*adfEnv.eFct)("adfMountHdFile : rootblock not found"); /*Edit PdV*/
        free ( dev->volList );
        dev->volList = NULL;
        free ( vol );
        dev->nVol = 0;
...
RETCODE adfMountHd ( struct AdfDevice * const dev )
{
    ...
    while( next!=-1 ) {
        ...
        if ( rc != RC_OK ) {
            ...
            //(*adfEnv.eFct)("adfMountHd : malloc"); /*Edit PdV*/
            return rc;
        }
        ...
        if (vList==NULL) {
            adfFreeTmpVolList(listRoot);
            //(*adfEnv.eFct)("adfMount : newCell() malloc");  /*Edit PdV*/
            return RC_MALLOC;
        }
    ...
    while( next!=-1 ) {
        ...
        if ( rc != RC_OK ) {
            for ( i = 0 ; i < dev->nVol ; i++ )
                free ( dev->volList[i] );
            free(dev->volList);
            // (*adfEnv.eFct)("adfMount : adfReadFSHDblock");  /*Edit PdV*/
            return rc;
        }
        next = fshd.next;
    }
    ...
    while( next!=-1 ) {
        ...
        if ( rc != RC_OK ) {
            //(*adfEnv.wFct)("adfMount : adfReadLSEGblock");  /*Edit PdV*/
            // abort here ?
        }
        next = lseg.next;
    }
...
/*struct AdfVolume *vol;*/
    ...
    if (!dev->volList) {
        //(*adfEnv.eFct)("adfCreateFlop : malloc"); /*Edit PdV*/
        return RC_MALLOC;
    }
    for(i=0; i<n; i++) {
...
/* pas fini */
           ...
           free(dev->volList);
           //(*adfEnv.eFct)("adfCreateHd : adfCreateVol() fails"); /*Edit PdV*/
           return RC_ERROR;
        }
