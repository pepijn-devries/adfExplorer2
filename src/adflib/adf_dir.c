 ...
 *
 */
//SECTNUM adfParentDir ( struct AdfVolume * const vol )  /* Edit PdV */
PREFIX RETCODE adfParentDir ( struct AdfVolume * const vol ) /* Edit PdV */
{
    struct bEntryBlock entry;
...
/*printf("readentry=%d\n",nSect);*/
    if (ent->checkSum!=adfNormalSum((uint8_t*)buf,20,512)) {
        //(*adfEnv.wFct)("adfReadEntryBlock : invalid checksum"); /*Edit PdV*/
        return RC_BLOCKSUM;
    }
    if (ent->type!=T_HEADER) {
        //(*adfEnv.wFct)("adfReadEntryBlock : T_HEADER id not found");  /*Edit PdV*/
        return RC_ERROR;
    }
    if ( ent->nameLen > MAXNAMELEN ||
         ent->commLen > MAXCMMTLEN )
    {
        //(*adfEnv.wFct)("adfReadEntryBlock : nameLen or commLen incorrect");  /*Edit PdV*/
        //printf("nameLen=%d, commLen=%d, name=%s sector%d\n",
        //    ent->nameLen,ent->commLen,ent->name, ent->headerKey);  /* Edit PdV */
    }
